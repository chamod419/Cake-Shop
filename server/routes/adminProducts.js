...
router.post("/", requireAdmin, upload.single("image"), async (req, res) => {
  try {
    ...
    const flavors = req.body?.flavors
      ? String(req.body.flavors).split(",").map((s) => s.trim()).filter(Boolean)
      : ["Chocolate"];
    ...
    const created = await prisma.product.create({
      data: { name, category, description, price, rating, image, sizes, flavors, isAvailable },
    });
...
router.put("/:id", requireAdmin, upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.body?.isAvailable !== undefined) data.isAvailable = String(req.body.isAvailable) === "true";

    if (req.body?.sizes !== undefined) {
      data.sizes = String(req.body.sizes).split(",").map((s) => s.trim()).filter(Boolean);
    }
    if (req.body?.flavors !== undefined) {
      data.flavors = String(req.body.flavors).split(",").map((s) => s.trim()).filter(Boolean);
    }
    ...
    if (req.file) {
      ...
      if (existing.image?.startsWith("/uploads/")) {
        ...
        fs.unlink(`uploads/${oldName}`).catch(() => {});
      }
    }
